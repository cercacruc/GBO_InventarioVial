...
class SurveyStorageTest {
    @Test fun versionFourUpgradeRetainsOldCulvertDimensionsAndMarksNewFieldsUnknown() = runBlocking {
        ...
        val db=Room.databaseBuilder(context,InventoryDatabase::class.java,name).addMigrations(InventoryDatabase.MIGRATION_4_5,InventoryDatabase.MIGRATION_5_6,InventoryDatabase.MIGRATION_6_7).build()
        try {
